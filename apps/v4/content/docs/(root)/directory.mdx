...
这些注册表已内置在 CLI 中，无需额外配置即可使用。要添加组件，请运行：`npx shadcn add @<注册表>/<组件>`。

<Callout className="bg-muted font-semibold">
  社区注册表由第三方开发者维护。安装时请务必检查代码，以确保符合您的安全与质量标准。
</Callout>
...
您可以使用 `shadcn` CLI 运行您自己的代码注册表。运行自己的注册表可以让您将自定义组件、hooks、页面、配置、规则和其他文件分发到任意项目中。
