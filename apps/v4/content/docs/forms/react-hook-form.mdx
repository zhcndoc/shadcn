...
In this guide, we’ll learn how to build forms using React Hook Form. This includes building forms with the `<Field />` component, adding schema validation with Zod, error handling, accessibility support, and more.
...
我们将从使用 Zod 模式定义表单的结构开始。

<Callout icon={<InfoIcon />}>
  **注意：** 此示例使用 `zod v3` 进行模式验证，但您可以使用 React Hook Form 支持的任何其他标准模式验证库替代。
</Callout>
