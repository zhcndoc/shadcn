      ...
      children={(field) => {
        ...
        return (
          <Field data-invalid={isInvalid}>
            <FieldLabel htmlFor={field.name}>漏洞标题</FieldLabel>
            <Input
              id={field.name}
              name={field.name}
...
const formSchema = z.object({
  title: z
    .string()
    .min(5, "Bug 标题长度必须至少为 5 个字符。")
    .max(32, "Bug 标题长度必须至多为 32 个字符。"),
  description: z
    .string()
    .min(20, "描述长度必须至少为 20 个字符。")
    .max(100, "描述长度必须至多为 100 个字符。"),
})
```
...
<form.Field
  ...
  children={(field) => {
    ...
    return (
      <Field data-invalid={isInvalid}>
        <FieldLabel htmlFor={field.name}>电子邮件</FieldLabel>
        <Input
          id={field.name}
          name={field.name}
          value={field.state.value}
...
<form.Field
  ...
  children={(field) => {
    ...
    return (
      <Field data-invalid={isInvalid}>
        <FieldLabel htmlFor="form-tanstack-input-username">用户名</FieldLabel>
        <Input
...
TanStack Form 提供了强大的数组字段管理功能，使用 `mode="array"`。您可以动态添加、移除和更新数组项，并支持完整的验证功能。
...
该示例演示如何使用数组字段管理多个电子邮件地址。用户最多可添加 5 个邮箱地址，且可删除单个地址，每个地址均独立验证。
...
使用括号语法访问单个数组项：`fieldName[index].propertyName`。此示例使用 `InputGroup` 将删除按钮与输入框并排显示。
...
使用 `field.pushValue(item)` 向数组字段添加项目。当数组达到最大长度时可以禁用按钮。
...
使用 `field.removeValue(index)` 从数组字段中移除项目。当项目数量超过一个时，可以有条件地显示删除按钮。
