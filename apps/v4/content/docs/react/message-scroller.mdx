...
有关行为指南和实时示例，请参阅
[消息滚动器](/docs/components/base/message-scroller) 组件。
...
外框和布局容器。它会填满其父容器，因此请将其放置在具有高度约束的布局中，并置于 `MessageScroller.Provider` 之内。

| Prop       | Type                          | Default | Description                        |
| ---------- | ----------------------------- | ------- | ---------------------------------- |
| `...props` | `React.ComponentProps<"div">` | -       | 传递给外框元素的 props。 |

根组件会同步以下滚动状态属性（viewport 也会带有这些属性），因此你可以根据滚动状态为容器设置样式，例如为外框添加边缘淡出效果。

| Data attribute       | Value                                             | Description                                                                                            |
| -------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `data-scrollable`    | `"start"` \| `"end"` \| `"start end"` \| _absent_ | 视口可滚动的方向。可通过 `[data-scrollable~="end"]` 查询其中一个方向；如果缺失，则表示内容刚好适配。 |
...
| `...props`        | `React.ComponentProps<"div">` | -             | 传递给内容元素的属性。                                           |
...
| ------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `start` | `boolean` | 视口是否可以向起始位置滚动。内容在上方被隐藏（`!start` 表示位于顶部）。                                                                          |
| `end`   | `boolean` | 视口是否可以向结束位置滚动。内容在下方被隐藏（`!end` 表示位于底部）。当 follow-output 将阅读器保持在实时边缘时，该值会保持为 `false`。 |
