...
- **`MessageScrollerContent`** — 对话内容容器。承载各行内容，并为新消息提供默认的实时区域。
...
`autoScroll` 会与轮次锚定协同工作。当新轮次锚定在靠近顶部的位置时，回复会在其下方流入对话区域，而视图会保持不动。当回复填满视口后，读者就会回到实时边缘，此时由跟随输出接替锚定继续工作。
...
它并结合你自己的标记和样式，请参阅
@shadcn/react 下的 [消息滚动器](/docs/react/message-scroller)。
