---
title: 表格 表格
description: 一个响应式表格组件。
component: true
---

<ComponentPreview name="table-demo" description="一个表格组件。" />

## 安装

<CodeTabs>

<TabsList>
  <TabsTrigger value="cli">命令行</TabsTrigger>
  <TabsTrigger value="manual">手动安装</TabsTrigger>
</TabsList>
<TabsContent value="cli">

```bash
npx shadcn@latest add table
```

</TabsContent>

<TabsContent value="manual">

<Steps className="mb-0 pt-2">

<Step>将以下代码复制并粘贴到你的项目中。</Step>

<ComponentSource
  name="table"
  title="components/ui/table.tsx"
  styleName="base-nova"
/>

<Step>更新导入路径以匹配你的项目结构。</Step>

</Steps>

</TabsContent>

</CodeTabs>

## 用法

```tsx showLineNumbers
import {
  Table,
  TableBody,
  TableCaption,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table"
```

```tsx showLineNumbers
<Table>
  <TableCaption>你最近发票的列表。</TableCaption>
  <TableHeader>
    <TableRow>
      <TableHead className="w-[100px]">发票</TableHead>
      <TableHead>状态</TableHead>
      <TableHead>付款方式</TableHead>
      <TableHead className="text-right">金额</TableHead>
    </TableRow>
  </TableHeader>
  <TableBody>
    <TableRow>
      <TableCell className="font-medium">INV001</TableCell>
      <TableCell>已支付</TableCell>
      <TableCell>信用卡</TableCell>
      <TableCell className="text-right">$250.00</TableCell>
    </TableRow>
  </TableBody>
</Table>
```

## 组合

使用以下组合来构建 `Table`：

```text
Table
├── TableCaption
├── TableHeader
│   └── TableRow
│       ├── TableHead
│       ├── TableHead
│       ├── TableHead
│       └── TableHead
├── TableBody
│   ├── TableRow
│   │   ├── TableCell
│   │   ├── TableCell
│   │   ├── TableCell
│   │   └── TableCell
│   └── TableRow
│       ├── TableCell
│       ├── TableCell
│       ├── TableCell
│       └── TableCell
└── TableFooter
```

## 页脚

更多信息请参考[数据表格](/docs/components/data-table)文档。

你也可以在[任务](/examples/tasks)演示中看到数据表格的示例。

<ComponentPreview styleName="base-nova" name="table-footer" />

## 操作

一个使用 `<DropdownMenu />` 组件为每一行显示操作的表格。

<ComponentPreview styleName="base-nova" name="table-actions" />

## 数据表格

你可以使用 `<Table />` 组件来构建更复杂的数据表格。将其与 [@tanstack/react-table](https://tanstack.com/table/latest) 结合使用，即可创建支持排序、筛选和分页的数据表格。

更多信息请参考[数据表格](/docs/components/data-table)文档。

你也可以在[任务](/examples/tasks)演示中看到数据表格的示例。

## 右到左（RTL）

若要在 shadcn/ui 中启用 RTL 支持，请参阅[RTL 配置指南](/docs/rtl)。

<ComponentPreview
  styleName="base-nova"
  name="table-rtl"
  direction="rtl"
  previewClassName="h-auto"
/>
