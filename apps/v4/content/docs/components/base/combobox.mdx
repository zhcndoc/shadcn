...
使用 `multiple` 进行多选，带芯片和芯片输入（请参见 [多选](#multiple)）。
...
在每个 `ComboboxGroup` 内使用 `ComboboxCollection` 按分组嵌套项目，组间有分隔符（请参见 [分组](#groups)）。
