...
- **SidebarContent** — 位于页眉和页脚之间的可滚动区域。
...
- **SidebarTrigger** — 用于切换侧边栏打开或折叠状态的控件。
...
使用 `SidebarFooter` 组件为侧边栏添加一个固定底部。
...
默认情况下，`SidebarMenuButton` 会渲染一个按钮，但你可以使用 `render` 属性来渲染其他组件，例如 `Link` 或 `a` 标签。
