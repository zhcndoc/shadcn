...
- **SidebarProvider** — 处理可折叠状态，并为子组件提供侧边栏上下文。
- **Sidebar** — 主要的可折叠侧边栏面板。
- **SidebarHeader** — 顶部固定；用于品牌标识、标题或工作区切换器。
- **SidebarFooter** — 底部固定；用于用户菜单、设置或操作。
- **SidebarContent** — 头部和脚部之间的可滚动区域。
- **SidebarGroup** — 将相关导航分组，可选标签、操作和内容区域。
- **SidebarMenu** / **SidebarMenuItem** — 用于链接、徽章、操作和嵌套子菜单的菜单结构。
- **SidebarRail** — 在适用时调整侧边栏宽度的调整手柄。
- **SidebarInset** — 使用 `inset` 变体时包裹主内容。
- **SidebarTrigger** — 切换侧边栏打开或折叠的控件。
...
`SidebarMenuAction` 组件用于在 `SidebarMenuItem` 内渲染菜单操作按钮。
