...
当某个注册表项需要将文件安装到用户配置的 shadcn 目录下时，请使用 `files[].target` 占位符。可用的占位符有 `@components/`、`@ui/`、`@lib/` 和 `@hooks/`。

这些占位符会根据 `components.json` 进行解析，因此同一个注册表项可以在使用 `@/`、自定义 TypeScript 别名、包导入或工作区包导出的项目中正常工作。

占位符后面的内容都会保留。例如，`@ui/ai/prompt-input.tsx` 会安装到用户配置的 `ui` 目录下的 `ai/prompt-input.tsx`。
...
注册表依赖项也可以使用目标占位符。以下示例中，子项会安装一个 UI 组件和一个辅助函数，而父项会安装一个应用组件和一个 Hook。
...
`target` 用于控制文件的写入位置，即使它与文件的 `type` 不同也是如此。
