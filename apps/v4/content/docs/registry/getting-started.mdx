...
- **HTML** 给浏览器——着陆页、文档或营销站点。
- **JSON** 给 `shadcn` CLI——可安装的注册表条目。
- **Markdown** 给 AI 代理和 LLM——内容的机器可读版本。
...
- **品牌化注册表 URL**：`shadcn add https://ui.example.com`
- **更短的 URL**：用户输入你的域名根路径，而不是 `/r/` 或 `/registry/` 子路径。
- **更容易记忆**：用户更容易记住并分享你的注册表。
