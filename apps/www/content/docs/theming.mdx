---
title: 主题
description: 使用 CSS 变量和颜色工具进行主题化。
---

您可以选择使用 CSS 变量（推荐）或工具类来进行主题设置。
...
我们使用简单的 `background` 和 `foreground` 约定来表示颜色。`background` 变量用于组件的背景色，`foreground` 变量用于文本色。

<Callout className="mt-4">

当变量用于组件的背景色时，会省略 `background` 后缀。
...
```tsx
<div className="bg-primary text-primary-foreground">你好</div>
```
...
要添加新颜色，您需要将它们添加到您的 CSS 文件和 `tailwind.config.js` 文件中。
...
有关在 Tailwind CSS 中使用颜色的更多信息，请参阅 [Tailwind CSS 文档](https://tailwindcss.com/docs/colors)。
